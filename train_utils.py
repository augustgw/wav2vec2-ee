...
class DKDEEWav2Vec2ForCTC(Wav2Vec2ForCTC):
    ...
    def forward(
        ...
        # for i in range(len(kd_loss)):
        #     joint_loss += ((self.ee_alpha[i] * ee_loss[i])/6) + ((1 - self.ee_alpha[i]) * kd_loss[i])
        # joint_loss += ((self.ee_alpha[i] * ee_loss[6])/6) # For final layer, no KD
